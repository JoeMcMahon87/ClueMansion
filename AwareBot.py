...
class CluedoBot:
    def __init__(self, ip):
        ...
        self.nickname = iter(['Prince Azul', 'Lady Lavender', 'Lord Gray', 'Miss Peach', 'Conan', 'Chimp'])
        self.user_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.user_socket.connect((self.host, 55555))
        # Possible Choices
        self.possible_answer = ["Colonel Mustard", "Professor Plum", "Mr. Green", "Mrs. Peacock", "Mrs. White", "Miss Scarlett", "Rusty", "Mrs. Meadow-Brook", 
        ...
        self.rooms = {1: "Tudor Mansion", 2: "Boat House", 3: "Gazebo", 4: "Swimming Pool", 5: "Stable", 6: "Gate House", 7:"Garden", 8:"Tennis Courts", 9:"Garage"}
        self.suspects = {1: "Colonel Mustard", 2: "Professor Plum", 3: "Mr. Green", 4: "Mrs. Peacock", 5: "Miss Scarlett",
            6: "Mrs. White", 7:"Rusty", 8:"Mrs. Meadow-Brook"}
        self.weapon = {1: "Horse Shoe", 2: "Hammer", 3: "Garden Shears", 4: "Water Bucket", 5: "Tennis Racquet", 6: "Lawn Gnome"}
    ...
    def bot_act_on_message(self, message):
        ...
        elif 'Choose a location to move to' in message:
            time.sleep(1)
            self.choose_room()
        elif '8.) Mrs. Meadow-Brook' in message:
            time.sleep(1)
            self.choose_suspect_and_weapon()
        elif '(S)earch or make (A)ccusation' in message:
            time.sleep(1)
            choice = random.choices(['a','s'], weights=(98, 2))
            self.send_bot_message(*choice)
        elif 'Do you want to reveal cards' in message:
            time.sleep(1)
            if winning:
                choice = random.choices(['y', 'n'], weights=(98, 2))
            else:
